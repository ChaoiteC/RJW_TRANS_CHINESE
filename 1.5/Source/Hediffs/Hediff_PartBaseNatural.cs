							...
							if (thisTick >= nextEggTick)
							{
								...
								if (eggedsize < maxEggsSize)
								{
									...
									if (genitals != null)
									{
										...
										neweggs = (float)(mineggs + (Rand.Value * (maxeggs - mineggs)) + 0.5);
										if (RJWSettings.DevMode)
										{
											ModLog.Message("Add Egg amount : " + neweggs + " int:" + (int)neweggs);
											ModLog.Message("Min : " + mineggs + " int:" + (int)mineggs);
											ModLog.Message("Max : " + maxeggs + " int:" + (int)maxeggs);
										}

										//Log.Message("-10 ");
										//Log.Message("genitals bpr: " + genitals);
										for (int i = 0; i < (int)neweggs; i++)
										{
